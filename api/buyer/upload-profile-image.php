...
$pdo = db();
if (!sukiwave_users_has_profile_image_column($pdo)) {
    json_response([
        'success' => false,
        'message' => 'Database migration required: profile image column is missing.',
    ], 503);
}
...
try {
    ...
    $relative = 'uploads/buyers/' . $basename;
    $base = sukiwave_public_api_root_url();
    $fullUrl = rtrim($base, '/') . '/' . $relative;

    $up = $pdo->prepare('UPDATE users SET profile_image_url = ? WHERE id = ?');
    $up->execute([$relative, $buyerUserId]);
